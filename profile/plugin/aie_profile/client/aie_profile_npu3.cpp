...
#include "xdp/profile/plugin/aie_profile/client/aie_profile_ct_writer.h"
...
namespace xdp {
  ...
  void AieProfile_NPU3Impl::updateDevice()
  {
    ...
    bool runtimeCounters = setMetricsSettings(metadata->getDeviceID());

    if (runtimeCounters && dtraceDebug) {
      AieProfileCTWriter ctWriter(db, metadata, metadata->getDeviceID());
      ctWriter.generate();
    }
  ...
  bool AieProfile_NPU3Impl::setMetricsSettings(const uint64_t deviceId)
  {
    ...
    for (int module = 0; module < metadata->getNumModules(); ++module) {
      ...
      for (auto& tileMetric : metadata->getConfigMetrics(module)) {
        ...
        // Skip interface tiles with empty stream_ids for bandwidth metrics
        ...
        if ((type == module_type::shim) && ((metricSet == "ddr_bandwidth") || (metricSet == "read_bandwidth") || (metricSet == "write_bandwidth"))) {
          numFreeCtr = static_cast<uint8_t>(tile.stream_ids.size());
        ...
        for (uint8_t i = 0; i < numFreeCtr; i++) {
          auto startEvent    = startEvents.at(i % startEvents.size());
          auto endEvent      = endEvents.at(i % endEvents.size());
  ...
  ***************************************************************************/
  ...
  uint64_t
  AieProfile_NPU3Impl::getCounterPayload(const tile_type& tile,
                                          ...
                                          uint8_t logicalPortIndex)
  {
    if (type == module_type::shim) {
      ...
      if ((metricSet == "ddr_bandwidth") || (metricSet == "read_bandwidth") || (metricSet == "write_bandwidth")) {
