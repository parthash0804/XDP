...
namespace xdp {
  ...
  void AieProfilePlugin::updateAIEDevice(void* handle, bool hw_context_flow)
  {
    ...
    // VE2 ELF Flow POC: when the hw_context was created from a Full ELF
    // (no xclbin), bypass the xclbin-style update path and let the ELF
    // itself drive the static-info update via ElfBinData.  All other
    ...
    if (hw_context_flow) {
      ...
      if (xrt_core::hw_context_int::get_elf_flow(ctx)) {
        ...
        try {
          ...
          // looking it up by a hardcoded kernel name.  The POC consumes
          // a single AIE-only ELF, so taking the last entry is fine
          // until multi-kernel selection is specified.
          auto elfMap = xrt_core::hw_context_int::get_elf_map(ctx);
          if (elfMap.empty()) {
            xrt_core::message::send(severity_level::warning, "XRT",
              "AIE Profile ELF POC: hw_context has no registered ELFs. "
              "Skipping ELF flow.");
            return;
          }
          ...
          xrt_core::message::send(severity_level::warning, "XRT",
            std::string("AIE Profile ELF POC: failed to acquire ELF: ") + e.what()
            + ". Skipping ELF flow.");
          return;
        }
#else
    ...
    // Skip when the ELF POC path already populated the device info
